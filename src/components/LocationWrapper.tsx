...
import { MapPin, AlertCircle, CheckCircle, XCircle, RefreshCw } from 'lucide-react';
...
const LocationWrapper: React.FC<LocationWrapperProps> = ({
  children,
  required = true,
  onLocationGranted,
  onLocationDenied,
  fallbackMessage = 'Lokasi diperlukan untuk melanjutkan'
}) => {
  ...
  // Jika permission sudah diberikan, tampilkan children
  if (permissionStatus === 'granted') {
    return <>{children}</>;
  }

  // Jika tidak required dan permission ditolak, tampilkan children dengan pesan
  if (!required && (permissionStatus === 'denied' || permissionStatus === 'error')) {
    return (
      <>
        {children}
        <div className="fixed bottom-4 right-4 bg-yellow-50 border border-yellow-200 rounded-lg p-3 shadow-lg max-w-sm">
          <div className="flex items-start space-x-2">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <p className="text-xs mt-1">{fallbackMessage}</p>
            </div>
          </div>
        </div>
      </>
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 sm:p-8">
        <div className="text-center">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">
            ...
            {permissionStatus === 'idle' && 'Izin Akses Lokasi'}
          </h2>
          ...
          <p className="text-gray-600 mb-6 leading-relaxed">
            ...
            {permissionStatus === 'idle' && 'Untuk memberikan layanan terbaik, kami memerlukan akses ke lokasi Anda. Lokasi akan digunakan untuk perhitungan estimasi dan pengiriman.'}
          </p>
