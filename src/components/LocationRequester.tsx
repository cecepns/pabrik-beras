...
import { MapPin, AlertCircle, CheckCircle, XCircle, RefreshCw, Shield, X } from 'lucide-react';
...
interface LocationRequesterProps {
  ...
  mode?: 'modal' | 'fullscreen' | 'button' | 'guard';
  title?: string;
  description?: string;
  allowSkip?: boolean;
  skipText?: string;
...
const LocationRequester: React.FC<LocationRequesterProps> = ({
  ...
  description = 'Untuk memberikan layanan terbaik, kami memerlukan akses ke lokasi Anda.',
  allowSkip = true,
  skipText = 'Lanjutkan Tanpa Lokasi',
  buttonText = 'Dapatkan Lokasi',
  ...
  useEffect(() => {
    if (mode === 'guard' || (mode === 'modal' && isOpen)) {
  ...
  const handleSkip = () => {
    ...
    toast('Anda melanjutkan tanpa lokasi. Beberapa fitur mungkin tidak optimal.');
  };
  ...
  const getButtonClasses = () => {
    ...
    const disabledClasses = permissionStatus === 'requesting' ? 'opacity-50 cursor-not-allowed' : '';

    return `${baseClasses} ${sizeClasses[buttonSize]} ${variantClasses[buttonVariant]} ${disabledClasses}`;
  };
  ...
  // Guard mode - jika permission sudah diberikan atau di-skip, tampilkan children
  if (mode === 'guard' && (permissionStatus === 'granted' || isSkipped)) {
    return <>{children}</>;
  }

  // Modal mode - jika tidak open, tidak tampilkan apa-apa
  if (mode === 'modal' && !isOpen) {
    return null;
  }

  // Fullscreen dan Modal content
  const content = (
  ...
  // Guard mode - tampilkan fullscreen content
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 sm:p-8">
        {content}
